...
//! listed or applied via the `strait template` subcommand. The template
//! registry doubles as the source data for the preset library exposed by
//! [`crate::presets`]: a preset is a named Cedar policy that a container
//! session can opt into at [`crate::grpc::StraitHostService::register_container`]
//! time. Keeping both views in this crate means the host process is the
//! single source of truth for the shipped Cedar corpus.
