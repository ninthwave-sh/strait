//! `strait launch --observe` orchestrator.
//!
//! Runs a command in a Docker container with full observation. All activity
//! is recorded to a JSONL observation log and streamed via Unix socket.
//!
//! Startup sequence:
//! 1. Verify Docker is running (fail fast)
//! 2. Start lightweight HTTPS proxy on a random port
//! 3. Write session CA to temp file
//! 4. Create container with bind-mounts (all read-write in observe mode)
//! 5. Inject CA into container's system bundle (via entrypoint wrapper)
//! 6. Set HTTPS_PROXY in container pointing to host proxy
//! 7. Start observation stream (JSONL file + Unix socket)
